...
module ItunesTransporterGenerator
	class Generator
		include FileHelpers
		...
		def initialize(output_to_file=true, id_prefix, provider, team_id, vendor_id)
					...
					achievement.locales.each do |locale|
						doc.locale('name' => locale.language) do
							...
							doc.after_earned_image() do
								...
								doc.size(get_file_size(image_name))
								doc.checksum(get_file_checksum(image_name), 'type' => 'md5')
							end
						end
					end
				end
		...
		def generate_metadata
			...
			leaderboards = []
			purchases = []
			output = @output_to_file ? File.new('metadata.xml', 'w') : $stdout
